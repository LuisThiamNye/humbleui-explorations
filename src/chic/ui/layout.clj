           ...
           (try
             (loop [y 0
               ...
               (if-some [[mode value child] (first children)]
                 (let [child-height (:height (case mode
                                               :hug (first known)
           ...
           (loop [dx 0
             ...
             (if-some [[mode value child] (first children)]
               (let [child-size (case mode
                                  :hug (first known)
                                  :stretch (if subpixel?
                                             (Point. (-> space (* value) (/ stretch)) 0)
                                             (IPoint. (-> space (* value) (/ stretch) (math/round)) 0)))
...
(deftype+ VScroll [child ^:mut offset ^:mut size ^:mut child-rect]
  IComponent
  (-measure [_ ctx cs]
            (measure-child child ctx (cui/unbounded-bottom cs)))

  (-draw [_ ctx cs ^Canvas canvas]
         (set! size cs)
         (let [child-height (:height (measure-child child ctx (cui/unbounded-bottom cs)))
               layer (.save canvas)]
...
(deftype+ HScroll [child ^:mut offset ^:mut size ^:mut child-rect]
  IComponent
  (-measure [_ ctx cs]
            (cui/rect-with-wh cs (measure-child child ctx (cui/unbounded-right cs))))

  (-draw [_ ctx cs ^Canvas canvas]
         (set! size cs)
         (let [child-width (:width (measure-child child ctx (cui/unbounded-right cs)))
               layer (.save canvas)]
...
(defn hscroll [child]
  (cui/dyncomp (->HScroll child 0 nil nil)))
